...
https://github.com/speechbrain/speechbrain/blob/develop/speechbrain/dataio/dataio.py#L755
...
from tidytunes.models.external.speechbrain_ecapa_tdnn.layers import BatchNorm1d, Conv1d
...
class SEBlock(nn.Module):
    ...
    def forward(self, x, lengths=None):
        ...
        if lengths is not None:
            mask = sequence_mask(lengths * L, max_length=L)
            mask = mask.unsqueeze(1)
            total = mask.sum(dim=2, keepdim=True)
            s = (x * mask.float()).sum(dim=2, keepdim=True) / total
        else:
